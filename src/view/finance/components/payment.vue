  ...
  <div class="app-container">
    ...
    <el-table class="table-consume-custom" v-loading="listLoading"
      ...
      <el-table-column align="center" label="营业收入" width="120">
        <template slot-scope="scope">{{ stats.business }}</template>
      </el-table-column>
      <el-table-column align="center" label="支付方式">
        <el-table-column v-for="(item,index) in stats.payments" :key="index" align="center" :label="item.payment_name" min-width="100">
          <template slot-scope="scope">{{ item.amount_price }}</template>
        </el-table-column>
      </el-table-column>
    </el-table>
  </div>
...
export default {
  ...
  methods: {
    getList () {
      ...
      this.$api.business.getPaymentStats(this.searchForm).then(res => {
        this.stats = res.data
