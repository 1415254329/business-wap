<template>
  <div class="app-container">
    ...
    <el-table class="table-consume-custom" v-loading="listLoading" :data="[stats]"
      ...
      <el-table-column align="center" label="快速消费">
        <template slot-scope="scope">{{ stats.speedy }}</template>
      </el-table-column>
      <el-table-column align="center" label="服务项目">
        <template slot-scope="scope">{{ stats.project }}</template>
      </el-table-column>
      <el-table-column align="center" label="商品销售">
        <template slot-scope="scope">{{ stats.commdity }}</template>
      </el-table-column>
      <el-table-column align="center" label="计次购买">
        <template slot-scope="scope">{{ stats.meter }}</template>
      </el-table-column>
    </el-table>
  </div>
</template>
...
export default {
  ...
  methods: {
    getList () {
      ...
      this.$api.business.getCategoryStats(this.searchForm).then(res => {
        this.stats = res.data
