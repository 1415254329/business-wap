<template>
  <div>
    ...
    <el-table :data="filterData" style="width: 100%">
      ...
      <el-table-column prop="validity_time" label="有效期"> </el-table-column>
      <el-table-column prop="validity_num" label="次数">
      </el-table-column>
...
<script>
import { getNumberList, delCommondity } from "@/api/scheduled.js";
import edit from "@/view/components/scheduled/comments/nummberedit";
  ...
  computed: {
    filterData: function () {
      ...
      this.tableData.forEach((item)=>{
          if(!item.status){
              item.validity_time?'':item.validity_time="无限期"
              array.push(item)
          }
      })
      return array;
  ...
  methods: {
    ...
    delItem(index) {
      ...
      this.$Modal.confirm({
        ...
        content: "是否确认删除？",
        onOk() {
          delCommondity(that.filterData[index]).then((res) => {
            if (!res.code) {
              this.$message({
                message: "删除成功",
                type: "success",
