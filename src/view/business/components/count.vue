<template>
  <div class="count">
    ...
    <el-table class="table-consume-custom" header-cell-class-name="el-table-header-cell-custom" :data="goodList" fit
      ...
      <el-table-column align="center" label="编号" min-width="65">
        <template slot-scope="scope">
          <div class="id">
            <Input type="number" @mousewheel.native.prevent @on-focus="showGoods($event,scope)"
              @on-enter="searchGood($event,scope.row)" v-model="scope.row.good.id" />
      ...
      <el-table-column align="center" label="名称" min-width="140">
        <template slot-scope="scope">
          <div class="name">
            ...
            <a v-if="Object.keys(scope.row.good).length" @click="useCount(scope.row,'buy')" class="useLink">使用</a>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="数量" min-width="55">
        <template slot-scope="scope">
    ...
    <el-table class="table-consume-custom" v-if="countUseList.length"
      header-cell-class-name="el-table-header-cell-custom" :data="countUseList" fit border>
      <el-table-column align="center" label="来源" min-width="60">
        <template slot-scope="scope">
          <span v-if="scope.row.type === 'countBuyUse'">购买订单</span>
          <span v-if="scope.row.type === 'countBoughtUse'">已购订单</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="编号" min-width="60">
        <template slot-scope="scope">
          {{scope.row.good.id}}
        </template>
      </el-table-column>
      <el-table-column align="center" label="名称" min-width="120">
        <template slot-scope="scope">
          {{scope.row.good.alias}}
        </template>
      </el-table-column>
      <el-table-column align="center" label="使用次数">
        <template slot-scope="scope">
          <Input @on-enter="inputEnterJump($event)" v-model.number="scope.row.useTimes" type="number"
            @mousewheel.native.prevent />
        </template>
      </el-table-column>
      <el-table-column v-for="(item,index) in staffSortList" :key="index" align="center" :label="item.alias">
    ...
    <Form :model="orderForm" :label-width="80" ref="orderForm" label-position="right">
      <FormItem label="支付方式">
        <div class="paymentList">
          <div v-for="(item,index) in !Object.keys(curMember).length?paymentList:memberPaymentList" :key="index" class="paymentList-item">
            <Input size="large" @on-enter="inputEnterJump($event)" v-model="item.amount_price" type="number" class="input-commission" @mousewheel.native.prevent>
      ...
      <div class="row">
        ...
        <Button class="btn-confirm" type="primary" size="large" :loading="loading" @click="submit">{{loading?'提交中':'立即结账'}}</Button>
      </div>
    </Form>
    <el-popover id="goodListPop" placement="bottom" trigger="manual" v-model="popGoodShow">
      <div v-for="(cItem,sIndex) in countList" :key="sIndex" class="chooseList-wrapper">
        ...
        <div class="chooseList">
          <div @click="selectGood(item,cItem)" v-for="(item,index) in cItem.children"
            :key="index">
            <el-tooltip effect="dark" :content="`${item.id} ${item.alias}`" placement="bottom" :disabled="`${item.id} ${item.alias}`.length <= 15">
...
export default {
  ...
  computed: {
    countBoughtUseList () {
      return this.$store.getters.countBoughtUseList || []
    }
  },
  watch: {
    countBoughtUseList () {
      this.checkBoughtCountUse()
    },
    curMember () {
      this.goodList.map(v => {
  ...
  methods: {
    ...
    getCategoryList () {
      return this.$api.common.getCategory().then((res) => {
        ...
        this.countList = this.categoryList.tenant_meter || {}
        this.$api.common.getMeter().then((res) => {
          ...
          Object.keys(this.countList).map(key => {
            this.countList[key].children = res.data[key] || []
          })
          this.countList = Object.values(this.countList)
          this.checkBoughtCountUse()
        })
      })
    },
    ...
    createGoodItem (type = 'count') {
      let goodItem = initGoodItem()
      goodItem.type = type
      goodItem.useTimes = 0
      goodItem.isInfinite = false
      goodItem.surp = -1
      goodItem.bonusList = JSON.parse(JSON.stringify(this.bonusList))

      if (['countBuyUse', 'countBoughtUse'].includes(type)) {
        return this.watchCountUse(goodItem)
      } else {
        return this.watchGood(goodItem)
      }
    },
    checkBoughtCountUse () {
      this.countBoughtUseList.map(v => {
        let find = this.countBoughtUseListCache.find(cv => cv.id === v.id)
        if (!find) {
          this.countBoughtUseListCache.push(v)
          let good = {
            bought: v,
            good: {
              id: v.meter_id,
              alias: v.alias,
              employ_cardinal_price: v.employ_cardinal_price
            },
            category: { id: v.meter_category_id || 0 }
          }
          if (!v.meter_category_id) {
            this.$Message.warning('未找到该计次分类，请手动输入员工业绩与提成')
          }
          this.useCount(good, 'bought')
        }
      })
      // function findCategory (list, id) {
      //   console.log(list, id)
      //   return list.find(v => v.good.id === id).category
      // }
    },
    computeGoodItem (obj) {
      ({
    ...
    computeGoodItemBonu (bonu, good) {
      if (!bonu.staff_id && !bonu.rule) {
        return
      }
      console.log(bonu, { ...good })
      let rule = JSON.parse(JSON.stringify(bonu.rule))
      let price = good.type === 'count'
        ? ((rule.type === 1) ? good.discountPriceAll : good.originalPriceAll) - good.presentPrice
        : good.good.employ_cardinal_price * good.useTimes
      let present = good.presentPrice || 0
      let result = calculate(rule, price, present, bonu.specified_switch)
    ...
    computePriceAll () {
      ...
      this.result.discountPriceAll = (Number(this.result.discountPriceAll) - Number(this.result.presentPriceAll)).toFixed(2)
      ...
      if (Object.keys(this.curMember).length) {
        this.orderForm.integral = Number(this.curMemberRule.consume_integral) ? Number((this.result.discountPriceAll + this.result.presentPriceAll) / this.curMemberRule.consume_integral).toFixed(2) : 0
    ...
    matchRule (v, type, categoryId, goodId) { // 根据会员等级,职位匹配计算规则   建议封装该函数，import引入
      if (!this.messageChokeFn) {
        this.messageChokeFn = messageChoke()
      }
      let ruleList = {}
      if (type === 'count') {
        ruleList = this.ruleList.tenant_meter || {}
      } else if (type === 'countBuyUse' || type === 'countBoughtUse') {
        ruleList = this.ruleList.tenant_meter_employ || {}
      }
      // 根据职位id获取规则
      if (Object.keys(v.cur_position).length) {
        ...
        if (isChild && ruleList.hasOwnProperty(v.cur_position.position_id) && ruleList[v.cur_position.position_id]
          ...
          this.messageChokeFn({
            type: 'warning',
            content: `50001:当前职位并未设置计次服务规则，请手动输入提成及业绩`
          })
          v.rule = {}
          return
        }
        ...
        if (!this.curMember.hierarchy) {
          if (v.rule['normal']) {
            ...
            this.messageChokeFn({
              type: 'warning',
              content: `50002:当前职位并未设置散客计次服务规则，请手动输入提成及业绩`
            })
            v.rule = {}
            return
          }
          ...
          if (v.rule[this.curMember.hierarchy]) {
            ...
            this.messageChokeFn({
              type: 'warning',
              content: `50003:当前职位并未设置会员计次服务规则，请手动输入提成及业绩`
            })
            v.rule = {}
            return
          }
        ...
        if (v.rule[categoryId]) {
          ...
          this.messageChokeFn({
            type: 'warning',
            content: `50004:当前职位并未设置该计次服务规则，请手动输入提成及业绩`
          })
          v.rule = {}
          return
        }
        ...
        if (v.rule[goodId]) {
          ...
          this.messageChokeFn({
            type: 'warning',
            content: `50005:当前职位并未设置该计次服务规则，请手动输入提成及业绩`
          })
          v.rule = {}
        }
      }
    ...
    watchGood (good) {
      ...
      good = new Proxy(good, {
        set (obj, prop, value, receiver) {
          ...
          if (prop === 'good') {
            obj.bonusList.map(v => {
              ...
              good.bonusList.forEach(v => {
                self.computeGoodItemBonu(v, good)
              })
            })
          }
          if (prop === 'qty') {
          ...
          if (prop === 'originalPrice') {
            obj.discountPrice = Number(obj.discount ? (obj.discount / goodPercent * obj.originalPrice) : obj.originalPrice).toFixed(2)
            computedItemPriceAll(obj)
            good.bonusList.forEach(v => {
              self.computeGoodItemBonu(v, good)
            })
      ...
      good.bonusList.forEach((v, i) => {
        good.bonusList[i] = new Proxy(v, {
          set: self.bonusProxySet.bind(this, good)
        })
      })
      return good
      ...
      function computedItemPriceAll (obj) {
        ...
        obj.discountPriceAll = Number(obj.discountPrice).toFixed(2)
        obj.discountPresentPrice = Number(Object.keys(self.curMemberRule).length
    ...
    watchCountUse (good) {
      ...
      good = new Proxy(good, {
        set (obj, prop, value, receiver) {
          obj[prop] = value
          if (prop === 'useTimes') {
            // let maxNum = self.goodList.reduce((pre, cur) => {
            //   if (cur.qty) return pre + cur.qty
            //   return pre
            // }, 0)
            // if (value > maxNum) {
            //   self.$nextTick(() => {
            //     obj[prop] = maxNum
            //   })
            //   self.$Message.info('使用次数不能大于总购买次数')
            // }
            good.bonusList.forEach(v => {
              self.computeGoodItemBonu(v, good)
            })
    ...
    bonusProxySet (good, obj, prop, value, receiver) {
      obj[prop] = value
      if (prop === 'show_num') {
        if (obj.show_num === '') {
          obj.staff_id = ''
          ...
          obj.dividends_price = ''
          // this.computeGoodItemBonu(obj, good)
        }
      } else if (prop === 'staff_id' || prop === 'cur_position') {
        if (obj.staff_id && Object.keys(obj.cur_position).length) {
          this.matchRule(obj, good.type, good.category.id, good.good.id)
          this.computeGoodItemBonu(obj, good)
          return true
        }
        ...
        Object.keys(obj.rule).length && (obj.dividends_price = obj.performance_price ? calculate(JSON.parse(JSON.stringify(obj.rule)),
    ...
    submit () {
      ...
      if (!Object.keys(this.curMember).length) {
        ...
        return
      } else if ((Object.keys(this.curMember).length && this.curMember.password) && this.memberPwd !== this.curMember.password) {
      ...
      // 若存在使用已购买计次的话消费金额限制解除
      let isBountUseExist = this.countUseList.some(v => v.type === 'countBoughtUse')
      if (this.orderForm.gross_price < 0 && !isBountUseExist) {
      ...
      this.orderForm.profit_price = this.orderForm.gross_price - (this.goodList.reduce((pre, cur) => {
        if (cur.good.cost_price) pre += cur.good.cost_price
        return pre
      }, 0))
      ...
      this.orderForm.meter_oredes = JSON.stringify(meterList.call(this))
      this.orderForm.meter_surplus = JSON.stringify(meterCountList.call(this))
      this.orderForm.shopping_cart = JSON.stringify(cartList.call(this))

      this.orderForm.bonus === '[]' && (this.orderForm.bonus = '')
      this.orderForm.meter_oredes === '[]' && (this.orderForm.meter_oredes = '')
      this.orderForm.meter_surplus === '[]' && (this.orderForm.meter_surplus = '')
      this.orderForm.shopping_cart === '[]' && (this.orderForm.shopping_cart = '')
      this.orderForm.payments === '[]' && (this.orderForm.payments = '')

      if (Object.keys(this.curMember).length) {
        this.orderForm.notify_format = initSMS('count', this.curMember,
      ...
      function bonusListAggregate () {
        let bonusList = []
        this.goodList.map(gv => {
          bonusList = bonusList.concat(JSON.parse(JSON.stringify(gv.bonusList.filter(v => {
            if (!v.staff_id) {
              return false
            }
        ...
        this.countUseList.map(cv => {
          bonusList = bonusList.concat(JSON.parse(JSON.stringify(cv.bonusList.filter(v => {
            if (!v.staff_id) {
              return false
            }
            let dividendsCache = v.dividends_price
      ...
      function meterList () {
        ...
        this.goodList.map(v => {
          if (Object.keys(v.good).length && v.good.id) {
            let findGood = result.find(fv => fv.meter_id === v.good.id)
            if (findGood) {
              findGood.validity_num += Number(v.qty)
            } else {
              let data = {
                meter_id: Number(v.good.id),
                alias: v.good.alias,
                exprice_time: v.good.validity_time,
                validity_num: Number(v.qty),
                employ_cardinal_price: Number(v.good.employ_cardinal_price),
                uses_num: 0
              }
              if (data.exprice_time) {
                data.exprice_time = parseTime(new Date().getTime() + 3600 * 24 * 1000 * Number(data.exprice_time))
              }
              result.push(data)
            }
          }
        ...
        this.countUseList.map(cv => {
          if (cv.type === 'countBuyUse') {
            let findItem = result.find(rv => rv.meter_id === cv.good.id)
            if (findItem) {
              findItem.uses_num += Number(cv.useTimes)
            }
          }
          if (cv.type === 'countBoughtUse') {
            result.push({
              id: cv.bought.id,
              uses_num: Number(cv.useTimes)
            })
          }
        })
        return result
      ...
      function meterCountList () {
        let result = []
        this.goodList.map(v => {
          if (Object.keys(v.good).length && v.good.id) {
            let findGood = result.find(rv => v.good && rv.self_id == v.good.id)
            if (findGood) {
              findGood.qty += 1
              findGood.price += Number(v.originalPriceAll)
            } else {
              result.push({
                self_id: Number(v.good.id),
                qty: 1,
                type: 'tenant_orders',
                price: Number(v.originalPriceAll)
              })
            }
          }
        })
      ...
      function cartList () {
        ...
        this.countUseList.map((v, i) => {
          let item = JSON.parse(JSON.stringify(v))

          // 获取剩余次数
          if (v.type === 'countBuyUse') {
            this.goodList.map((gv, i) => {
              if (!gv.good.id) {
                return
              }
              if (gv.good.id === v.good.id) {
                item.isInfinite = (Number(gv.qty) === 0) && true
                item.surp = (Number(gv.qty) === 0) ? 0 : (Number(gv.qty) - Number(v.useTimes))
              }
            })
          } else if (v.type === 'countBoughtUse') {
            this.countBoughtUseList.map(bv => {
              if (bv.id === v.bought.id) {
                item.isInfinite = (Number(bv.validity_num) === 0) && true
                item.surp = (Number(bv.validity_num) === 0) ? 0 : (Number(bv.validity_num) - Number(bv.uses_num) - Number(v.useTimes))
              }
            })
          }
          delete item.bonusList
    ...
    selectGood (obj, cObj) {
      ...
      if (!this.goodList.some(v => !Object.keys(v.good).length)) {
        ...
        this.$nextTick(function () {
          this.inputList = Array.from(document.querySelector('.count').querySelectorAll(
            'input.ivu-input'))
        }.bind(this))
      }
    ...
    removeGood (index) {
      ...
      // 移除方式?
      // this.countUseList.splice(this.countUseList.findIndex(v => v.goodRowIndex === index), 1)
    },
    useCount (good, type = 'buy') {
      let countUseIndex = this.countUseList.findIndex(v => (v.good.id === good.good.id) && (v.type === 'countBuyUse'))
      if ((type === 'buy' && countUseIndex < 0) || (type === 'bought')) {
        let goodItem = this.createGoodItem(type === 'buy' ? 'countBuyUse' : 'countBoughtUse')
        goodItem.index = setCountUseIndex.call(this)
        goodItem.category = good.category
        goodItem.good = good.good
        if (type === 'bought')goodItem.bought = good.bought
        goodItem.originalPrice = good.good.employ_cardinal_price || 0
        goodItem.useTimes = 1
        goodItem.bonusList.map(v => {
          v.type = 'tenant_meter_employ'
          v.source_alias = good.good.alias
          v.source_id = good.good.id
          v.source_weight = goodItem.index
        })
        this.countUseList.push(goodItem)
      } else {
        this.$Message.info('使用表单中已存在该项目')
      }

      function setCountUseIndex () {
        return (this.countUseList.reduce((pre, cur) => {
          if (pre < cur.index) return cur.index
          return pre
        }, 0) + 1)
      }
      this.$nextTick(function () {
        this.inputList = Array.from(document.querySelector('.count').querySelectorAll('input.ivu-input'))
      }.bind(this))
    },
    ...
    reset () {
      ...
      this.$set(this.goodList, 0, this.createGoodItem())
      this.countUseList = []
      this.$store.commit('business/setCountBoughtUseList', [])
      this.paymentCache = {}
      this.paymentList.map(v => {
        v.amount_price = 0
      })
      ...
      this.order_time = ''
      this.memberPwd = ''
      this.countBoughtUseListCache = []
    }
  },
  destroyed () {
    if (this.countBoughtUseList.length) {
      this.$store.commit('business/setCountBoughtUseList', [])
    }
  }
}
