<template>
  <el-autocomplete v-model="searchText" size="medium" :fetch-suggestions="querySearchAsync" placeholder="请输入内容"
    @select="handleSelect" popper-class="search-suggestion" :popper-append-to-body="false" highlight-first-item>
    ...
    <template slot-scope="scope">
      <div v-if="scope.item.alias" class="item">
        ...
        <span class="ellipsis-1">{{scope.item.serial_num || '暂无数据'}}</span>
  ...
  methods: {
    getMemberList () {
      this.$api.common.getMemberList().then((res) => {
        this.curMemberList = res.data && Object.values(res.data).filter((val) => val.status === 0)
    ...
    querySearchAsync (queryString, cb) {
      ...
      if (!this.loading) { // 如果会员数据仍在请求，延迟执行
        setTimeout(() => {
      ...
      let memberList = this.memberList.length ? this.memberList : this.curMemberList
