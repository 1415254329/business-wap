  ...
  <Row :gutter="16">
    ...
    <Card>
      ...
      <RadioGroup :value="curEmployeeNum" type="button" @on-change="setEmployeeNum" class="radio-button-ver">
        <Radio  :label="1">单人开卡提成设置</Radio>
        <Radio  :label="2">两人开卡提成设置</Radio>
        <Radio  :label="3">三人开卡提成设置</Radio>
      </RadioGroup>
    </Card>
    </Col>
    <Col span="10">
    <Card>
      ...
      <Form :model="calculationRule" :label-width="80" ref="calculationRule">
        <FormItem label="计算类型" class="radio-inside">
          <RadioGroup v-model="calculationRule[curEmployeeNum].employees.type">
  ...
  methods: {
    init () {
      ...
      this.$api.user.getRules().then(res => {
        this.allRules = res.data.tenant_member || {}
      })
    },
    positionSelected (v) {
    ...
    setEmployeeNum (num) {
      ...
      //   if (this.calculationRule.employees.length < num) {
      //     for (let i = 0; i < (num - this.calculationRule.employees.length); i++) {
      //       this.calculationRule.employees[this.calculationRule.employees.length] = JSON.parse(JSON.stringify(cardItem))
      //     }
      //   } else {
      //     for (let i = 0; i < (this.calculationRule.employees.length - num); i++) {
      //       this.calculationRule.employees.pop()
      //     }
      //   }
      this.$forceUpdate()
    },
