<template>
  <Row :gutter="16">
    ...
    <Card>
      ...
      <div class="menu">
        ...
        <div class="menu-level2">
          <div v-for="(item,index) in project[curCategoryId]" :key="index" class="menu-level2-item"
            :class="{'active':curProjectId == item.id}" @click="projectSelect(item)">
            {{item.alias}}
          </div>
        </div>
      </div>
    ...
    <Card>
      <div>规则设置</div>
      <Form v-if="calculationRule[curCategoryId] && calculationRule[curCategoryId][curProjectId]"
        :model="calculationRule[this.curCategoryId][this.curProjectId]" :label-width="80" ref="calculationRule">
        <FormItem label="提成基数" class="radio-inside">
          <RadioGroup v-model="calculationRule[curCategoryId][curProjectId].type">
        ...
        <Card class="mt">
          <p slot="title">提成</p>
          <p class="mt">指定</p>
          <Input v-model="calculationRule[curCategoryId][curProjectId].commission.specified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curProjectId].commission.specified.type" slot="prepend"
            class="radio-inside">
          ...
          <p class="mt">非指定</p>
          <Input v-model="calculationRule[curCategoryId][curProjectId].commission.unspecified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curProjectId].commission.unspecified.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
        ...
        <Card class="mt">
          <p slot="title">业绩</p>
          <p class="mt">指定</p>
          <Input v-model="calculationRule[curCategoryId][curProjectId].perform.specified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curProjectId].perform.specified.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
          ...
          <p class="mt">非指定</p>
          <Input v-model="calculationRule[curCategoryId][curProjectId].perform.unspecified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curProjectId].perform.unspecified.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
        ...
        <Card class="mt">
          <p slot="title">赠金业绩</p>
          <Input v-model="calculationRule[curCategoryId][curProjectId].grants.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curProjectId].grants.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
        ...
        <Card class="mt">
          <p slot="title">成本</p>
          <p class="mt">指定</p>
          <Input v-model="calculationRule[curCategoryId][curProjectId].cost.specified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curProjectId].cost.specified.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
          ...
          <p class="mt">非指定</p>
          <Input v-model="calculationRule[curCategoryId][curProjectId].cost.unspecified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curProjectId].cost.unspecified.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
        ...
        <Card class="mt">
          <p slot="title">二次成本</p>
          <p class="mt">指定</p>
          <Input v-model="calculationRule[curCategoryId][curProjectId].twice.specified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curProjectId].twice.specified.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
          ...
          <p class="mt">非指定</p>
          <Input v-model="calculationRule[curCategoryId][curProjectId].twice.unspecified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curProjectId].twice.unspecified.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
  ...
  methods: {
    init () {
      ...
      this.$api.user.getRules().then(res => {
        this.allRules = res.data.tenant_project || {}
      })

      this.$api.common.getCategory().then((res) => {
        this.category = Object.values(res.data['tenant_project']) || []
        this.curCategoryId = this.category[0].id
      })
    ...
    saveRule (form) {
      ...
      if (this.curProjectId === 'all') {
        this.project[this.curCategoryId].forEach(v => {
