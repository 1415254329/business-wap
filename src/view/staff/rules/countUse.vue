<template>
  <Row :gutter="16">
    ...
    <Card>
      ...
      <div class="menu">
        ...
        <div class="menu-level2">
          <div v-for="(item,index) in counts[curCategoryId]" :key="index" class="menu-level2-item"
            :class="{'active':curCountId == item.id}" @click="countsSelect(item)">
            {{item.alias}}
          </div>
        </div>
      </div>
    ...
    <Card>
      <div>规则设置</div>
      <Form v-if="calculationRule[curCategoryId] && calculationRule[curCategoryId][curCountId]"
        :model="calculationRule[this.curCategoryId][this.curCountId]" :label-width="80" ref="calculationRule">
        <FormItem label="提成基数" class="radio-inside">
          <RadioGroup v-model="calculationRule[curCategoryId][curCountId].type">
        ...
        <Card class="mt">
          <p slot="title">提成</p>
          <p class="mt">指定</p>
          <Input v-model="calculationRule[curCategoryId][curCountId].commission.specified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curCountId].commission.specified.type" slot="prepend"
            class="radio-inside">
          ...
          <p class="mt">非指定</p>
          <Input v-model="calculationRule[curCategoryId][curCountId].commission.unspecified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curCountId].commission.unspecified.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
        ...
        <Card class="mt">
          <p slot="title">业绩</p>
          <p class="mt">指定</p>
          <Input v-model="calculationRule[curCategoryId][curCountId].perform.specified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curCountId].perform.specified.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
          ...
          <p class="mt">非指定</p>
          <Input v-model="calculationRule[curCategoryId][curCountId].perform.unspecified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curCountId].perform.unspecified.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
        ...
        <Card class="mt">
          <p slot="title">赠金业绩</p>
          <Input v-model="calculationRule[curCategoryId][curCountId].grants.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curCountId].grants.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
        ...
        <Card class="mt">
          <p slot="title">成本</p>
          <p class="mt">指定</p>
          <Input v-model="calculationRule[curCategoryId][curCountId].cost.specified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curCountId].cost.specified.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
          ...
          <p class="mt">非指定</p>
          <Input v-model="calculationRule[curCategoryId][curCountId].cost.unspecified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curCountId].cost.unspecified.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
        ...
        <Card class="mt">
          <p slot="title">二次成本</p>
          <p class="mt">指定</p>
          <Input v-model="calculationRule[curCategoryId][curCountId].twice.specified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curCountId].twice.specified.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
          ...
          <p class="mt">非指定</p>
          <Input v-model="calculationRule[curCategoryId][curCountId].twice.unspecified.value" type="number">
          <RadioGroup v-model="calculationRule[curCategoryId][curCountId].twice.unspecified.type" slot="prepend"
            class="radio-inside">
            <Radio :label="1">比例</Radio>
            <Radio :label="2">固定</Radio>
          </RadioGroup>
...
export default {
  ...
  watch: {
    ...
    curCountId () {
      if (!this.calculationRule[this.curCategoryId][this.curCountId]) {
        this.calculationRule[this.curCategoryId][this.curCountId] = JSON.parse(JSON.stringify(servicesItem))
      }
    }
  },
  mounted () {
  ...
  methods: {
    init () {
      ...
      this.$api.user.getRules().then(res => {
        this.allRules = res.data.tenant_meter_employ || {}
      })

      this.$api.common.getCategory().then((res) => {
        this.category = Object.values(res.data['tenant_meter'] || {})
    ...
    saveRule (form) {
      ...
      if (this.curCountId === 'all') {
        this.counts[this.curCategoryId].forEach(v => {
